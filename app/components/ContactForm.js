export function ContactForm() {
  ...
  <legend>Envianos tus comentarios</legend>
        ...
        <input type="email" name="email" placeholder="Escribe tu correo" title="Email incorrecto" required
            pattern="^[a-z0-9]+(\\.[_a-z0-9]+)*@[a-z0-9-]+(\\.[a-z0-9-]+)*(\\.[a-z]{2,15})$">
        <input type="text" name="subject" placeholder="Asunto a tratar" title="El asunto es requerido" required>
        <textarea name="comments" placeholder="Escribe tu comentario" cols="50" rows="5" data-pattern="^.{1,255}$"
  ...
  function validationsForm() {
    ...
    d.addEventListener("keyup", (e) => {
      if (e.target.matches(".contact-form [required]")) {
        ...
        if (!pattern) {
          //console.log("El no input tiene patron");
          return $input.value === ""
            ? d.getElementById($input.name).classList.add("is-active")
            : d.getElementById($input.name).classList.remove("is-active");
        }
    ...
    d.addEventListener("submit", (e) => {
      ...
      setTimeout(() => {
        ...
        $form.reset();
        setInterval(() => $response.classList.add("none"), 3000);
      }, 3000);
    });
  }
